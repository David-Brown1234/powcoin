...
#if defined(NDEBUG)
# error "PowCoin cannot be compiled without assertions."
#endif
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // PowCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // PowCoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // PowCoin: 1 minutes
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1405666800; // 2014-07-18 15:00
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084524493;

        if (fTestNet)
        {
            block.nTime    = 1405666800;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // PowCoin: increase each by adding 2 to bitcoin's value.
...
void static PowCoinMiner(CWallet *pwallet)
{
    printf("PowCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PowCoin-miner");
    ...
    try { loop {
        ...
        printf("Running PowCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PowCoinMiner, pwallet));
}
